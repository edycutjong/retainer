...
test.describe("input validation — the same answer for every malformed request", () => {
  ...
  test("the gate refuses an agent that is not an EVM address", async ({
    request,
  }) => {
    const response = await request.get(
      "/api/retainer/access?agent=not-an-address",
    );
    expect(response.status()).toBe(400);
  });

  test("the read-only status route refuses a request with no agent", async ({
    request,
  }) => {
  ...
  test("the read-only status route refuses an agent that is not an EVM address", async ({
    request,
  }) => {
...
test.describe("fail closed — an unpaid agent never receives the resource", () => {
  test("a cold agent is never served, whatever goes wrong upstream", async ({
    request,
  }) => {
    const response = await request.get(
      `/api/retainer/access?agent=${COLD_AGENT}`,
    );
...
test.describe("live gate (opt-in)", () => {
  ...
  test("the deployed gate challenges a cold agent on hedera:testnet", async ({
    request,
  }) => {
    const response = await request.get(
      `${liveBase}/api/retainer/access?agent=${COLD_AGENT}`,
    );
